pub fn generate_async_result_handler() -> proc_macro2::TokenStream {
    quote::quote! {
        ...
        async fn _kybra_async_result_handler_call(vm: &rustpython::vm::VirtualMachine, py_object_ref: &PyObjectRef, args: &Vec<PyObjectRef>) -> PyObjectRef {
            ...
            let call_result_tuple = match &cross_canister_call_function_name[..] {
                "_kybra_call_ManagementCanister_raw_rand" => {
                    // TODO dynamically add params, calling try_from_vm_value on args
                    create_call_result_tuple(vm, _kybra_call_ManagementCanister_raw_rand(canister_id_principal).await)
                },
                _ => panic!("cross canister function does not exist")
            };

            _kybra_async_result_handler(vm, py_object_ref, call_result_tuple.into()).await
        }
        ...
        async fn _kybra_async_result_handler_call_raw(vm: &rustpython::vm::VirtualMachine, py_object_ref: &PyObjectRef, args: &Vec<PyObjectRef>) -> PyObjectRef {
            ...
            _kybra_async_result_handler(vm, py_object_ref, create_call_result_tuple(vm, call_raw_result).into()).await
        }
        ...
        fn create_call_result_tuple<T>(vm: &rustpython::vm::VirtualMachine, call_result: CallResult<T>) -> PyObjectRef
            where T: for<'a> CdkActTryIntoVmValue<&'a rustpython::vm::VirtualMachine, rustpython::vm::PyObjectRef>
        {
            match call_result {
                Ok(ok) => {
                    vm.ctx.new_tuple(vec![ok.try_into_vm_value(vm).unwrap(), vm.ctx.none()]).into()
                ...
                Err(err) => {
                    ...
                    vm.ctx.new_tuple(vec![vm.ctx.none(), err_string.try_into_vm_value(vm).unwrap()]).into()
        ...
        // TODO remove this once cross canister functions are generated automatically
