...
pub fn generate() -> TokenStream {
    quote::quote! {
            ...
            fn try_from_vm_value(
                ...
                if self.is(&vm.ctx.none()) {
                    ...
                    Err(CdkActTryFromVmValueError(format!(
                        "TypeError: expected NoneType but received {type_name}"
                    )))
                }
            }
        }
        ...
        impl CdkActTryFromVmValue<bool, &rustpython::vm::VirtualMachine> for rustpython::vm::PyObjectRef {
            fn try_from_vm_value(
                ...
                match self.try_into_value(vm) {
                    Ok(value) => Ok(value),
                    Err(err) => Err(CdkActTryFromVmValueError(format!(
                        "TypeError: expected bool but received {type_name}"
            ...
            for rustpython::vm::PyObjectRef
        {
            ...
            ) -> Result<ic_cdk::export::candid::Empty, CdkActTryFromVmValueError>
            {
                Err(CdkActTryFromVmValueError(
                    "TypeError: PyObjectRef cannot be converted into Empty".to_string(),
                ))
            }
        }
            ...
            for rustpython::vm::PyObjectRef
        {
            ...
            ) -> Result<ic_cdk::export::candid::Func, CdkActTryFromVmValueError>
            {
                ...
                let principal = match tuple_self.get(0) {
                    ...
                    None => {
                        return Err(CdkActTryFromVmValueError(
                            "TypeError: could not convert value to Func, missing Principal"
                                .to_string(),
                        ))
                    }
                };
                let method = match tuple_self.get(1) {
                    ...
                    None => {
                        return Err(CdkActTryFromVmValueError(
                            "TypeError: could not convert value to Func, missing method"
                                .to_string(),
                        ))
                    }
                };
        ...
        impl CdkActTryFromVmValue<ic_cdk_timers::TimerId, &rustpython::vm::VirtualMachine>
            for rustpython::vm::PyObjectRef
        {
            fn try_from_vm_value(
                self,
        ...
        impl CdkActTryFromVmValue<String, &rustpython::vm::VirtualMachine> for rustpython::vm::PyObjectRef {
            fn try_from_vm_value(
                ...
                match self.try_into_value(vm) {
                    ...
                    Err(err) => {
                        Err(CdkActTryFromVmValueError(format!(
                            "TypeError: expected str but received {type_name}"
                        ))) // TODO consider using the try_into_value err
                    }
                }
            }
        ...
        impl CdkActTryFromVmValue<Result<(), String>, &rustpython::vm::VirtualMachine>
            for rustpython::vm::PyObjectRef
        {
            fn try_from_vm_value(
                self,
            ...
            ) -> Result<Result<(), String>, CdkActTryFromVmValueError>
            {
                ...
                let type_name = self.to_pyobject(vm).class().name().to_string();
                Err(CdkActTryFromVmValueError(format!(
                    "TypeError: expected Result but received {type_name}"
                )))
            }
        }
    }
