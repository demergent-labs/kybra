...
state: State = {"accounts": {}, "name": "", "ticker": "", "total_supply": 0}


@update
def initialize_supply(
    name: str, original_address: str, ticker: str, total_supply: nat64
) -> bool:
    global state

    state = {
        "accounts": {
            original_address: {"address": original_address, "balance": total_supply}
        },
        "name": name,
        "ticker": ticker,
...
@update
def transfer(from_address: str, to_address: str, amount: nat64) -> bool:
    if state["accounts"].get(to_address, None) is None:
        state["accounts"][to_address] = {"address": to_address, "balance": 0}
