...
from .candid_types import Post
from .reactions import get_reaction_from_state_reaction
from .state import state, StatePost, StateThread, StateUser
...
def create_post(
    ...
    updated_state_thread = get_updated_state_thread(
        thread_id,
        state_post['id']
    )
...
@query
def get_all_posts(join_depth: nat32) -> list[Post]:
    return list(
        map(
...
def get_post_from_state_post(
    ...
    if join_depth == 0:
        ...
        reactions = list(
            map(
                lambda state_reaction: get_reaction_from_state_reaction(state_reaction, join_depth),
                state_reactions
            )
        )
