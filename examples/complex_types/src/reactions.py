...
def create_reaction(
    ...
    updated_state_post = get_updated_state_post(
        post_id,
        state_reaction['id']
    )
    ...
    reaction = get_reaction_from_state_reaction(
        state_reaction,
        join_depth
    )
...
@query
def get_all_reactions(join_depth: nat32) -> list[Reaction]:
    return list(
        map(
